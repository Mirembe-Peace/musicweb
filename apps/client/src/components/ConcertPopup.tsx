...
export default function ConcertPopup() {
  ...
  useEffect(() => {
    const fetchActiveConcert = async () => {
      try {
        const res = await axios.get(`${API_URL}/concerts/active`);
        if (res.data) {
          setConcert(res.data);
        ...
        console.error("Failed to fetch active concert", error);
      }
    };
    fetchActiveConcert();
  }, []);
        ...
        >
          <Card className="rounded-3xl border shadow-2xl bg-card overflow-hidden">
            ...
            <CardContent className="p-6">
              ...
              <Button className="w-full rounded-2xl h-12 font-bold shadow-brand">
                Buy Tickets Now
              </Button>
            </CardContent>
